...
resource "docker_container" "this" {
  ...
  entrypoint = length(var.container_data.entrypoint) > 0 ? var.container_data.entrypoint : null
  ...
  dynamic "capabilities" {
    for_each = var.container_data.capabilities != null && (length(try(var.container_data.capabilities.add, [])) > 0 || length(try(var.container_data.capabilities.drop, [])) > 0) ? [var.container_data.capabilities] : []
    content {
  ...
  dynamic "upload" {
    ...
    content {
      ...
      source_hash    = upload.value.source != null ? filemd5(upload.value.source) : upload.value.source_hash
